...
[`golem`](https://thinkr-open.github.io/golem/) Shiny application. It
...
- Country Benchmarking: Compares institutional indicator performance for
  a base country against comparator countries or predefined groups using
  closeness-to-frontier (CTF) scores.

- Modular Architecture: Each dashboard tab is a self-contained Shiny
  module (`mod_*.R`), wired together through a single golem `app_ui()` /
  `app_server()` pair. Shared reactive state is passed between modules
...
- Reproducible Data: All indicator data is sourced at runtime from the
  ...
  package — no data files are bundled in this repository.

- One-Command Deployment: `deploy_app()` regenerates the Posit Connect
  entry file with the current runtime options baked in, then publishes
  it, so a chosen data vintage survives Connect’s own restart and
  scale-out schedule.
...
This also installs `cliaretl` from GitHub (declared in `Remotes:`),
...
For development, restore the locked environment and use the golem
workflow:
...
The dashboard is deployed to Posit Connect via `deploy_app()`. It reads
...
- `closeness_to_frontier_static` / `closeness_to_frontier_dynamic` — CTF
  scores
- `db_variables_final` — indicator metadata
- `wb_country_groups` / `wb_country_list` — country and group reference
  tables
