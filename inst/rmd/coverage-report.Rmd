...
## dplyr/stringr/tidyr/ggplot2/knitr are already reachable here via
## cliarappak's own imports (mod_reports.R renders with envir parented at
## the package namespace). flextable is the one real dependency this Rmd
## needs that isn't -- it's a `Suggests`, not an `Import`, so it still
## needs its own library() call. The rest of the original block
## (tidyverse, readxl, here, haven, tibble, sf, janitor, countrycode,
## RColorBrewer) was leftover boilerplate from the original script app --
## none of them are actually called anywhere in this file, and tidyverse
## specifically isn't even installed here (deliberately -- see
...
To provide context for the analysis of data coverage, it is essential to understand the update frequency and timing of the data sources that construct the indicators used in the CLIAR framework. The table below summarizes the latest year available on the dashboard for each manual data source, along with their expected publication cycles. This information helps to assess the timeliness of the data and identify potential limitations in the analysis.

| Manual Data Extraction (sources)   | Latest year on dashboard | Expected Publication cycle |
|------------------------------------|--------------------|----------------------------------|
| Fraser Institute                   | 2022                     | Annual                     |
| Heritage Index of Economic Freedom | 2024                     | Annual                     |
| Open Budget Survey                 | 2023                     | Bi-annually                |
| RISE                               | 2021                     | Bi-annually                |
| Romelli                            | 2023                     | Academic Source            |
| SPI                                | 2023                     | Bi-annually                |
| V-DEM                              | 2024                     | Annually                   |
...
availability_ctf_long <- params$ctf_static_long %>%
  ...
  mutate(
    availability = ifelse(is.na(value), "missing", "available")
  )
...
ctf_dynamic_clean <- params$ctf_dynamic %>%
  # remove family indicators and non-family indicators
  filter(
    !str_detect(indicators, "_avg$") &
      family_name != "(other indicators)" &
      !is.na(family_name)
    )
...
for(family_name in unique_family_names) {
  print(
    generate_availability_heatmap(
      ctf_dynamic_clean, 
      country_name_input = params$base_country, 
      family_name
    )
  )
}
